...
class Validator(object):
    ...
    def split_deftok(self,in_tok,device):
        if(len(str(in_tok))>1):#aplly to all words
            try:
                intok,pos_word,word,dict_pos=in_tok.split('|_',3)
            except:
                print('intok wrong: ',in_tok)#,' the prts are: ',intok,pos_word,word,dict_pos)       
            intok=torch.tensor(int(intok)).to(device)
        else:#paddings BOS EOS case
            intok=torch.tensor(int(in_tok)).to(device)
            word=''
            pos_word=''
            dict_pos=''                 
        return (intok,pos_word,word,dict_pos) 
    ...
    def evaluate_perp(self, model):
        ...
        with torch.no_grad():
            for batch_data in self.data_manager.get_batch(mode=ac.VALIDATING):
                ...
                src_toks_cuda = src_toks_temp.to(device)
                trg_toks_cuda = trg_toks_temp.to(device)
                targets_cuda = targets_temp.to(device)


                # get loss
                ret = model(src_toks, src_toks_cuda, trg_toks, trg_toks_cuda, targets_cuda, device)
                total_loss += ret['nll_loss'].cpu().detach().numpy()
                total_smoothed_loss += ret['loss'].cpu().detach().numpy()
                total_weight += (targets_cuda != ac.PAD_ID).cpu().detach().numpy().sum()
        ...
        with torch.no_grad():
            ...
            for (src_toks, original_idxs) in self.data_manager.get_trans_input(src_file):
                ...
                for i, ret in enumerate(rets):
                    ...
                    #print('\n\n\n Symbols at validation here:',symbols,'\n\n')
